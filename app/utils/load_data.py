...
def load_data(client):
    ...
    if not client.indices.exists(index=index_name):
        logger.warning(
            "Индекс '%s' не существует. Пропускаем загрузку данных.", index_name
        )
        return
    ...
    if doc_count > 0:
        logger.info(
            "В индексе '%s' уже есть %s документов. Пропускаем загрузку.",
            index_name,
            doc_count,
        )
        return

    logger.info("Загружаю начальные данные в индекс '%s'...", index_name)
    ...
    for doc in docs:
        ...
        logger.info("Добавлен: %s [%s]", doc["title"], doc["content_type"])
...
if __name__ == "__main__":
    ...
    if opensearch_client.success:
        load_data(client=opensearch_client.credentials)
    else:
